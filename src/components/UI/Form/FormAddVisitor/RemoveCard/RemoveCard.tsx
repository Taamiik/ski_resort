...
import { fetchRemoveVisitor } from '../../../../../store/visitors/actions';
	...
	const confirmDeletion = () => {
		return (
			<div className={styles.cardData}>
				<p>Вы уверены, что хотите удалить карточку этого посетителя?</p>
			</div>
		)
	}
	
	const removeCard = () => {
		dispatch(fetchRemoveVisitor([data.id, paramPage]))
		closedForm()
	}
	...
	return (
		<div onClick={stopPropagation} className={styles.wrapper}>
			<div className={styles.header}>
				<div><span>Удаление посетителя</span></div>
